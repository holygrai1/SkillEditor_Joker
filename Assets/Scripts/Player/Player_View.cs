...
public class Player_View : MonoBehaviour
{
    ...
    private GameData customCharacterData;                    // 玩家自定义的角色数据-用于存档
    ...
    public void Init(GameData gameData)
    {
        ...
        this.customCharacterData = gameData;
    }

    /// <summary>
    /// 游戏中的初始化
    /// </summary>
    public void InitOnGame(GameData gameData)
    {
        animation.Init();
        Init(gameData);
        // 基于数据设置当前部位
        CharacterPartConfigBase faceConfig = ConfigTool.LoadCharacterPartConfig(CharacterPartType.Face, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Face].Index);
        CharacterPartConfigBase clothCofig = ConfigTool.LoadCharacterPartConfig(CharacterPartType.Cloth, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Cloth].Index);
        CharacterPartConfigBase hairConfig = ConfigTool.LoadCharacterPartConfig(CharacterPartType.Hair, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Hair].Index);
        CustomCharacterPartData facePartData = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Face];
    ...
    public void SetPart(CharacterPartConfigBase characterPartConfig, bool updateCharacterView = true)
    {
        ...
        {
            // 释放旧配置的资源
            ResSystem.UnloadAsset<CharacterPartConfigBase>(currPartConfig);
