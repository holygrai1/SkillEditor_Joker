...
[UIWindowData(nameof(UI_CreateCharacterWindow), false, nameof(UI_CreateCharacterWindow), 2)]
public class UI_CreateCharacterWindow : UI_WindowBase
{
    ...
    // 自定义角色的数据
    private GameData gameData => DataManager.GameData;
    ...
    {
        // 获取配置
        projectConfig = ResSystem.LoadAsset<ProjectConfig>(ConfigTool.ProjectConfigName);
        ...
        lastPosXOnDragModel = modelTouchImage.transform.position.x;
        modelTouchImage.OnDrag(ModelTouchImageDrag);
        modelTouchImage.OnEndDrag(ModelTouchImageEndDrag);
        ...
        // 应用默认的部位
        SetCharacterPart(CharacterPartType.Face, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Face].Index, true, true);
        SetCharacterPart(CharacterPartType.Hair, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Hair].Index, false, true);
        SetCharacterPart(CharacterPartType.Cloth, gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Cloth].Index, false, true);
    ...
    // 最后一次坐标
    float lastPosXOnDragModel;
    /// <summary>
    /// 当模型交互区域拖拽时的回调
    /// </summary>
    private void ModelTouchImageDrag(PointerEventData eventData)
    {
        if (lastPosXOnDragModel == float.MaxValue)
        {
    ...
    private void ModelTouchImageEndDrag(PointerEventData eventData)
    {
        lastPosXOnDragModel = float.MaxValue;    // 约定的无效数
    }
    ...
    {
        gameData.ProfessionType = professionType;
        // 处理实际的职业切换逻辑
    ...
    public void SetCharacterPart(CharacterPartType partType, int configIndex, bool updateUIView = false, bool updateCharacterView = false)
    {
        ...
        if (updateUIView)
        {
            ...
            switch (partType)
            {
                case CharacterPartType.Face:
                    ...
                    heightSlider.value = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Face].Height;
                    heightSlider.minValue = 0;
                    heightSlider.maxValue = 0.1f;

                    // 尺寸
                    sizeSlider.transform.parent.gameObject.SetActive(true);
                    sizeSlider.value = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Face].Size;
                ...
                case CharacterPartType.Hair:
                    ...
                    if ((partConfig as HairConfig).ColorIndex != -1)
                    {
                        ...
                        Color color = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Hair].Color1.ConverToUnityColor();
                        // 让颜色按钮的图片和当前配置一样
                ...
                case CharacterPartType.Cloth:
                    ...
                    {
                        color1Button.gameObject.SetActive(true);
                        Color color = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Cloth].Color1.ConverToUnityColor();
                        // 让颜色按钮的图片和当前配置一样
                        color1Button.image.color = new Color(color.r, color.g, color.b, 0.6f);
                    }
                    else color1Button.gameObject.SetActive(false);
                    ...
                    if ((partConfig as ClothConfig).ColorIndex2 != -1)
                    {
                        ...
                        Color color = gameData.CustomPartDataDic.Dictionary[(int)CharacterPartType.Cloth].Color2.ConverToUnityColor();
                        // 让颜色按钮的图片和当前配置一样
        ...
        // 保存数据
        gameData.CustomPartDataDic.Dictionary[(int)partType].Index = configIndex;
    }
    ...
    private void SetNextChracterPart(bool isLeft, bool updateUI, CharacterPartType currPartType)
    {
        ...
        while (!partConfig.ProfessionTypes.Contains(professionType))
        {
            ...
            // 释放资源
            ResSystem.UnloadAsset<CharacterPartConfigBase>(partConfig);
        ...
        AudioSystem.PlayOneShot(arrowClickAudioClip, Vector3.zero, false, 1);
    }
    ...
    {
        // 显示颜色选择器窗口
        UISystem.Show<UI_ColorSelectorWindow>().Init(OnColor1Seletec, color1Button.image.color);
    }

    private void Color2ButtonClick()
    {
        // 显示颜色选择器窗口
        UISystem.Show<UI_ColorSelectorWindow>().Init(OnColor2Seletec, color1Button.image.color);
    }
    ...
    private CustomCharacterPartData GetCharacterPartData()
    {
        ...
        // 数据层面部位数据
        CustomCharacterPartData partData = gameData.CustomPartDataDic.Dictionary[(int)currPartType];
        return partData;
    }
